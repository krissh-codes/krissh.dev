...
export default function CompanyLogo({ companyName, customSize = 150, className }: { companyName: string; customSize?: number, className?: string }) {
    ...
    return <img className={className} src={`/images/logos/${companyName}/${themeMode}.svg`} alt={`${companyName}`} width={customSize} height={customSize} loading="lazy" style={{ height: 'auto' }} />;
}
