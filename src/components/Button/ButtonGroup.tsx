...
export function ButtonGroup(props: ButtonGroupProps) {
    ...
    return (
        <div className={classes.button_group} role="group" aria-label="Statistics range selector">
            {props.buttonItems.map(buttonItem => (
                <button key={buttonItem.id} id={buttonItem.id} className={classes.button_group__item} onClick={onButtonClick} aria-pressed={selectedItem?.id === buttonItem.id}>
                    {buttonItem.icon}
                    {buttonItem.label}
                </button>
            ))}
